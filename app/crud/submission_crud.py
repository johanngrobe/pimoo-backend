...
from app.services.pdf import BasePDF

ModelType = TypeVar("ModelType")
CreateSchemaType = TypeVar("CreateSchemaType")
UpdateSchemaType = TypeVar("UpdateSchemaType")

class CRUDSubmission(CRUDBase[ModelType, CreateSchemaType, UpdateSchemaType]):
    ...
    async def copy_nested(
        ...
        for attr, sub_attrs in nested_attrs.items():
            ...
            for item in nested_items:
                ...
                setattr(copied_item, 'submission', copied_instance)  # Set parent reference

                # Recursively copy sub-nested items if sub-attributes are provided
                if sub_attrs:
                    await self.copy_nested(item, copied_item, {sub_attr: [] for sub_attr in sub_attrs}, exclude)
    ...
    async def copy(
        ...
        if nested_attributes:
            await self.copy_nested(instance, copied_instance, nested_attributes, exclude)
